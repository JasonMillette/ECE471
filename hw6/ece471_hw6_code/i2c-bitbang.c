...
static int SDA_gpio_pull_low(void) {
	...
	/* and then write the value '0' to sda_fd		  */
        ...
        result = write(sda_fd, "0", 1);                         //writes 0 to sda
        if (result < 0) {
            fprintf(stderr, "Error in SDA_gpio_pull_low\n");    //error checking
            return -1;
        }
...
static int SCL_gpio_pull_low(void) {
	...
	/* and then write the value '0' to sda_fd		*/
        ...
        result = write(scl_fd, "0", 1);                        //writes 0 to scl
        if (result < 0) {
            fprintf(stderr, "Error in SCL_gpio_pull_low\n");    //error checks
            return -1;
        }
...
static int read_SDA(void) {
	...
	/* remember to convert from ASCII before returning.	*/
        ...
        gpio_set_input(SDA_GPIO);
        lseek(sda_fd,0,SEEK_SET);                           //backtracks file
        read(sda_fd, temp, 1);
        value = atoi(temp);
...
static int read_SCL(void) {
	...
	/* remember to convert from ASCII before returning.	*/
        ...
        gpio_set_input(SCL_GPIO);
        lseek(scl_fd,0,SEEK_SET);                           //backtracks file
        read(scl_fd, temp, 1);
        value = atoi(temp);
...
static int i2c_write_byte(unsigned char byte) {
	...
	int nack=1;
        ...
        for (int i = 7; i >= 0; i--) {
            i2c_write_bit(((byte >> i) & ~0xFE));      //Shifts to the needed bit
        }                                           //then clears other bits

        nack = i2c_read_bit();                          //returns acknowledge bit
	return nack;
}
