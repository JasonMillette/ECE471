...
_start:
	...
	mov	r0,#0
        MOV     R6,#15  //used for loop counter
...
print:
	...
	@                                        \/
	@==========================================
        CMP R6, #0      //Checks for last loop
        BEQ exit        //Exits on last loop
        SUB R6, R6, #1  //decrements loop number
        MOV R0, R6      //sets R0 to loop number
        B print         //loops for next line
	@==========================================
...
loop:
        LDRB R8, [R1]   //Loads the character
        CMP R8, #0      //checks for NULL termination
        BEQ end         //If NULL end
