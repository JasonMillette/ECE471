...
int main(int argc, char **argv) {
	...
	/* Once per second.					*/
        while (1) {
            ...
            data_out[2] = 0x00;         //setting for channel 2
            data_out[1] = 0xA0;
            data_out[0] = 0x01;
            ...
            float degc = (100 * vin) - 50;                          //converts voltage to degrees C
            float degf = (degc * (9/5)) + 32;                       //convrets degrees C to F
            if(degf >= 100)  printf("Its hot out!\n");              //conditional prints
            if (degf <= 32) printf("Its freezing out!\n");
            printf("temperature:%f\u2109\n",degf);                  //prints degrees F
